...
/*-------------------------------------------------------------------------------
< Gen_Shared_Key >
: When Secret Key from MCU is generated, this function is for creating Shared Key
  by using SHA1. The important this is that sha1 make a 20bytes data. We use only
  16bytes as a shared key
			uint8_t* aesKey = shared key buffer
			uint8_t* secMCU = secret key from MCU
			int len = length of secMCU
-------------------------------------------------------------------------------*/
void Gen_Shared_Key(uint8_t* aesKey, uint8_t* secMCU, int len) {
	uint8_t* secKey;
	Sha1.init();
	for (int i = 0; i<len; i++) {
		Sha1.write(secMCU[i]); //secret key를 Sha1으로 Hash
	}
	secKey = Sha1.result();
	memcpy(aesKey, secKey, 16);
	return;
}