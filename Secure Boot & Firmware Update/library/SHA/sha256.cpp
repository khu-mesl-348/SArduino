...
/*-------------------------------------------------------------------------------------
< Sha256_Hash >
: This function is a first stage for generating PIC Code. note that this function
reads binary from start address to end address of application service in flash, and
use SHA256 algorithm to generate a hash value
			unsigned long startAddress = start address of application service binary
			unsigned long endAddress = end address of application service binary
			uint8_t* hash_buffer = the data buffer which is used for storing hash value
---------------------------------------------------------------------------------------*/
void Sha256_Hash(unsigned long startAddress, unsigned long endAddress, uint8_t* hash_buffer)
{
	int       theValue;
	...
	Sha256.init();
	while (myAddressPointer != endAddress)
	{
		theValue = pgm_read_byte_near(myAddressPointer);
		Sha256.write(theValue);
		myAddressPointer++;
	}
	uint8_t* sendBuffer = Sha256.result();
	for (int i = 0; i < 32; i++) hash_buffer[i] = sendBuffer[i];
